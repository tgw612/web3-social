...
impl UserService {
    ...
    async fn find_or_create_user(
        ...
        let user_entity: Vec<UserEntity> =
            UserEntity::select_by_column(&rb, "wallet_address", wallet_address_val)
                .await
                .map_err(|e| ServiceError::InternalServerError)?;
        if let Some(entity) = user_entity.first() {
            // 将实体转换为User模型
            ...
            // 创建新用户实体
            let new_user_entity: UserEntity = UserEntity {
                id: uuid::Uuid::new_v4(),
                username: "".to_string(),
    ...
    pub async fn update_profile(
        ...
        if let Some(username_val) = &username {
            // 使用rbatis查询用户名是否已存在
            let wrapper: bool = self.db.new_wrapper().eq("username", username_val);

            let count = self
                .db
                .fetch_count_by_wrapper::<UserProfileEntity>(wrapper)
                .await
            ...
            if count > 0 {
                ...
                self.db
                    .update_by_wrapper::<UserProfileEntity>(&update_values, wrapper)
                    .await
                    .map_err(|_| ServiceError::InternalServerError)?;
    ...
    pub async fn get_profile(&self, user_id_val: String) -> Result<UserProfile, ServiceError> {
        // 使用rbatis查询用户资料
        let wrapper = self.db.new_wrapper().eq("user_id", &user_id_val);

        let profile = self
            .db
            .query_by_wrapper::<UserProfileEntity>(wrapper)
            .await
            .map_err(|_| ServiceError::InternalServerError)?
    ...
    pub async fn get_profile_by_username(
        ...
        // 使用rbatis查询用户资料
        let wrapper = self.db.new_wrapper().eq("username", username_val);
    ...
    pub async fn get_wallet_address_by_user_id(
        ...
        // 使用rbatis查询用户
        let wrapper = self.db.new_wrapper().eq("id", user_id_uuid);

        let user_entity = self
            .db
            .fetch_by_wrapper::<UserEntity>(wrapper)
            .await
            .map_err(|_| ServiceError::InternalServerError)?
            .ok_or(ServiceError::NotFound("用户不存在".into()))?;

        Ok(user_entity.wallet_address)
    }
}
